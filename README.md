...
CyberKitchen is a fast, 100% client-side security and developer toolkit that runs entirely in your browser. It bundles encryption, hashing, key derivation, encoding, number conversion, authentication tokens and data generators into one installable, offline-first app. No servers, no tracking, and no data ever leaves your machine, so every tool works with no network after the first load.
